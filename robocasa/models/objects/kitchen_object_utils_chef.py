...
import robocasa
from robocasa.models.objects.kitchen_objects_chef import OBJ_CATEGORIES, OBJ_GROUPS
...
def sample_kitchen_object_helper(
    ...
    if isinstance(groups, str) and groups.endswith(".xml"):
        ...
        # 对于特定的单个组（如 "apple"），选择固定的对象
        if len(groups) == 1 and groups[0] in OBJ_CATEGORIES:
            cat = groups[0]
            # 选择第一个可用的注册表
            for reg in obj_registries:
                if reg in OBJ_CATEGORIES[cat] and OBJ_CATEGORIES[cat][reg].mjcf_paths:
                    # 选择该类别中的第一个对象（你可以修改索引来选择其他对象）
                    mjcf_path = OBJ_CATEGORIES[cat][reg].mjcf_paths[
                        1
                    ]  # 使用 [0] 选择第一个，[1] 选择第二个，以此类推
            ...
            for reg in obj_registries:
                ...
                if split is not None:
                    split_th = max(
                        len(choices) - 3, int(math.ceil(len(reg_choices) / 2))
                    )
                    if split == "A":
