...
from robocasa.models.objects.kitchen_object_utils_chef import sample_kitchen_object
from robocasa.models.objects.objects import MJCFObject
from robocasa.utils._placement_samplers_chef import (
...
class KitchenChef(ManipulationEnv, metaclass=KitchenEnvMeta):
    """
    ...
    # from robocasa_extension.utilities.debug_tools import debug_placement_initializer
    # @debug_placement_initializer

    def _get_placement_initializer(self, cfg_list, z_offset=0.01):
        """Creates a placement initializer for the objects/fixtures"""
        print("\n=== 创建放置初始化器 ===")
        # print(f"配置列表: {[cfg.get('name', 'unknown') for cfg in cfg_list]}")
        # print(f"配置列表: {cfg_list}")
        ...
        for (obj_i, cfg) in enumerate(cfg_list):
            try:
                # print(f"\n处理对象 {obj_i}: {cfg.get('name', 'unknown')}")

                # 规范化配置
                if "type" not in cfg:
                    print(f"警告: 配置缺少type字段")
                    continue
                ...
                try:
                    mj_obj = (
                        self.fixtures[cfg["name"]]
                        if obj_type == "fixture"
                        else self.objects[cfg["name"]]
                    )
                    # print(f"获取到对象: {mj_obj}")
                except KeyError:
                    print(f"警告: 找不到对象 {cfg['name']}")
                    continue

                placement = cfg.get("placement", None)
                if placement is None:
                    # print("警告: 没有placement配置")
                    continue
                # print(f"放置配置: {placement}")

                # 获取 fixture
                fixture_id = placement.get("fixture", None)
                if fixture_id is None:
                    print(f"Warning: No fixture specified for object {cfg['name']}")
                    continue
                ...
                try:
                    ...
                    # print(f"获取到的fixture: {fixture}")
                except Exception as e:
                    # print(f"Error getting fixture for {cfg['name']}: {e}")
                    continue
                ...
                try:
                    ...
                    print(f"Error sampling reset region for {cfg['name']}: {e}")
                    continue
                ...
                if macros.SHOW_SITES is True:
                    ...
                    pos_to_vis[:2] += T.rotate_2d_point(
                        [reset_region["offset"][0], reset_region["offset"][1]],
                        rot=ref_rot,
                    )
                ...
                # print(f"添加采样器成功: {cfg['name']}")

            except Exception as e:
                print(f"处理对象 {cfg.get('name', 'unknown')} 失败: {str(e)}")
                import traceback

                traceback.print_exc()
                continue

        print(f"\n放置初始化器创建完成，包含 {len(placement_initializer.samplers)} 个采样器")
    ...
    def get_fixture(self, id, ref=None, size=(0.2, 0.2)):
        ...
        if ref is None:
            # this is the part that involves multiple counter regions
            ...
            # change the random selection to select the second counter region
            if len(matches) > 1:
                key = matches[0]  # select the second matching counter
            else:
                key = matches[0]  # if only one, select it
    ...
    def get_obj_lang(self):
        obj_lang = []
        if isinstance(self.obj_cfg, dict):
            for obj_name, obj_cfg in self.obj_cfg.items():
                if (
                    obj_cfg is None
                    or "info" not in obj_cfg
                    or "cat" not in obj_cfg["info"]
                ):
                    print(
                        f"Warning: Invalid or incomplete configuration for object {obj_name}"
                    )
                    continue
                lang = obj_cfg["info"]["cat"].replace("_", " ")
                obj_lang.append(f"{obj_name}: {lang}")
        elif isinstance(self.obj_cfg, list):
            for obj_cfg in self.obj_cfg:
                if (
                    isinstance(obj_cfg, dict)
                    and "info" in obj_cfg
                    and "cat" in obj_cfg["info"]
                ):
                    lang = obj_cfg["info"]["cat"].replace("_", " ")
                    obj_lang.append(f"{obj_cfg.get('name', 'Unknown')}: {lang}")
                else:
                    print(f"Warning: Invalid or incomplete configuration for an object")
        else:
            print(f"Warning: Unexpected type for self.obj_cfg: {type(self.obj_cfg)}")
        return obj_lang
    ...
    #             print(f"Counter: {name}, Position: {fixture.pos}")
    #         else:
    #             print(f"Fixture: {name}, Type: {type(fixture)}, Position: {fixture.pos if hasattr(fixture, 'pos') else 'N/A'}")
